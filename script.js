...
function highlightCurrentNav() {
    ...
    navLinks.forEach(link => {
        ...
        if (!linkHref) return;
        if (!linkHref.startsWith('#') && (linkHref === currentPage || linkHref === window.location.pathname)) {
...
// Run on load, on scroll, and on hashchange
window.addEventListener('load', highlightCurrentNav);
window.addEventListener('scroll', debounce(highlightCurrentNav, 80));
